# train_hf_multilabel_codet5.py — Stable on old transformers + copy artifacts to runs/ (Method A)
...
# - เพิ่ม: คัดลอกผลลัพธ์สำคัญไปไว้ใน OUT_DIR/runs/ เพื่อเปิดดูง่าย
...
OUT_DIR   = Path(r"D:\GitHub\dolap\files_dolab\dataset_gpa\model_codet5_10e_8b_m512")  # <= เปลี่ยนชื่อโฟลเดอร์ผลลัพธ์ได้ตามต้องการ

LABELS  = ["inputError", "processError", "outputError"]
MAX_LEN = 512
BATCH   = 8
EPOCHS  = 10              # คุณตั้งเป็น 1 รอบก่อน ถ้าพอใจแล้วค่อยเพิ่มเป็น 3
LR      = 5e-5
...
class T5EncoderForMultiLabel(nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs):
        ...
        # คืนแบบมาตรฐาน HF เพื่อให้ Trainer จัดการ concatenation ได้เสถียร
...
def main():
    ...
    # ---------- 13) Copy artifacts to runs/ (Method A) ----------
    ...
    def _safe_copy(src: Path, dst: Path):
        try:
            ...
            print(f"[WARN] copy {src.name} -> runs failed: {e}")

    _safe_copy(OUT_DIR / "best_thresholds.json", runs_dir / "best_thresholds.json")
    _safe_copy(OUT_DIR / "test_classification_report.txt", runs_dir / "test_classification_report.txt")
    for lb in LABELS:
        _safe_copy(OUT_DIR / f"confusion_matrix_{lb}.png", runs_dir / f"confusion_matrix_{lb}.png")
    _safe_copy(save_dir / "pytorch_model_custom_head.bin", runs_dir / "pytorch_model_custom_head.bin")
    _safe_copy(save_dir / "meta.json", runs_dir / "meta.json")
    _safe_copy(save_dir / "labels.json", runs_dir / "labels.json")

    print(f"\nArtifacts copied to: {runs_dir}")
