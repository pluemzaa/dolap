num = input("Enter a series of numbers separated by commas:")
numt = num.split(",")

numt0 = int(numt[0])
numt1 = int(numt[1])
numt2 = int(numt[2])
numt3 = int(numt[3])
numt4 = int(numt[4])

numf0 =  (numt[0] is max(numt) in numt0)
numf1 =  (numt[1] is max(numt) in numt1)
numf2 =  (numt[2] is max(numt) in numt2)
numf3 =  (numt[3] is max(numt) in numt3)
numf4 =  (numt[4] is max(numt) in numt4) 

print(numt0, "is the maximum value:", numf0)
print(numt1, "is the maximum value:", numf1)
print(numt2, "is the maximum value:", numf2)
print(numt3, "is the maximum value:", numf3)
print(numt4, "is the maximum value:", numf4)